...
public class PictureSelectorCameraEmptyActivity extends PictureBaseActivity {
    ...
    private boolean isUseDoodle; //是否使用涂鸦
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        isUseDoodle = getIntent().getBooleanExtra("isUseDoodle", false);
...
//        config.cameraPath = isAudio ? getAudioPath(intent) : config.cameraPath;
        ...
        if (isUseDoodle) {
            ...
            final String cameraOutDir = BoxingFileHelper.getExternalDCIM(BoxingFileHelper.DEFAULT_SUB_DIR);
            File outputFile = new File(cameraOutDir, System.currentTimeMillis() + ".jpg");*/
            Uri uri = Uri.fromFile(new File(config.cameraPath));
            Intent intent1 = new Intent(this, IMGEditActivity.class);
            intent1.putExtra(IMGEditActivity.EXTRA_IMAGE_URI, uri);
//            intent1.putExtra(IMGEditActivity.EXTRA_IMAGE_SAVE_PATH, outputFile.getPath());
            intent1.putExtra(IMGEditActivity.EXTRA_IMAGE_SAVE_PATH, config.cameraPath);
            startActivityForResult(intent1, WeXin_REQUEST_CODE);
        } else {
            showPleaseDialog();
